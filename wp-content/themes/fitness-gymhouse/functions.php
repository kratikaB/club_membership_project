...
function fitness_gymhouse_widgets_init() {
	...
	for ($i=1; $i<=$fitness_gymhouse_widget_areas; $i++) {
		register_sidebar( array(
			'name'          => __( 'Footer ', 'fitness-gymhouse' ) . $i,
			'id'            => 'footer-' . $i,
			'description'   => '',
...
function fitness_gymhouse_scripts() {
	...
	// blocks-css
	wp_enqueue_style( 'fitness-gymhouse-block-style', get_theme_file_uri('/assets/css/blocks.css') );
...
function fitness_gymhouse_sanitize_phone_number( $phone ) {
	return preg_replace( '/[^\d+]/', '', $phone );
}

function fitness_gymhouse_sanitize_checkbox( $input ) {
	return ( ( isset( $input ) && true == $input ) ? true : false );
}

function fitness_gymhouse_sanitize_float( $input ) {
	return filter_var($input, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
}

function fitness_gymhouse_sanitize_number_range( $number, $setting ) {
...
if (!function_exists('fitness_gymhouse_loop_columns')) {
	function fitness_gymhouse_loop_columns() {
		$columns = get_theme_mod( 'fitness_gymhouse_woocommerce_product_per_columns', 3 );
		return $columns; // 3 products per row
	}
}

//Change number of products that are displayed per page (shop page)
add_filter( 'loop_shop_per_page', 'fitness_gymhouse_shop_per_page', 20 );
function fitness_gymhouse_shop_per_page( $cols ) {
  	$cols = get_theme_mod( 'fitness_gymhouse_woocommerce_product_per_page', 9 );
	return $cols;
}
...
define('FITNESS_GYMHOUSE_LIVE_DEMO',__('https://themeseye.com/demo/gymhouse/','fitness-gymhouse'));
define('FITNESS_GYMHOUSE_BUY_PRO',__('https://www.themeseye.com/wordpress/premium-fitness-wordpress-theme/','fitness-gymhouse'));
define('FITNESS_GYMHOUSE_PRO_DOC',__('http://themeseye.com/theme-demo/docs/gymhouse-pro/','fitness-gymhouse'));
define('FITNESS_GYMHOUSE_FREE_DOC',__('http://themeseye.com/theme-demo/docs/gymhouse/','fitness-gymhouse'));
define('FITNESS_GYMHOUSE_PRO_SUPPORT',__('https://www.themeseye.com/forums/forum/themeseye-support/','fitness-gymhouse'));
define('FITNESS_GYMHOUSE_FREE_SUPPORT',__('https://wordpress.org/support/theme/fitness-gymhouse/','fitness-gymhouse'));
//footer Link

define('FITNESS_GYMHOUSE_CREDIT',__('https://www.themeseye.com/wordpress/free-fitness-wordpress-theme/','fitness-gymhouse'));
