  ...
  jQuery( document ).ready(function() {
	...
	jQuery( "#wpeditorwidget-html" ).click(function() {
	 ...
	 jQuery("#show_msg").show();
				...
				<p><a href="javascript:WPEditorWidget.updateWidgetAndCloseEditor(true);" id="save_btn" class="button button-primary"><?php esc_html_e( 'Save and close', 'complete' ); ?></a></p>
                <p id="show_msg"><strong style="color:#F00;"><?php esc_html_e('Click On "Visual" Tab To Save Content.', 'complete');?></strong></p>
			</div>
		</div>
...
function complete_customizer_stuff() {
        ...
        <div id="complete_settings">
        	...
        	<h3><?php esc_attr_e('SETTINGS', 'complete'); ?></h3>
            
            <!--RESET OPTIONS-->
            <div id="reset_options" class="setting_option">
            	<h4><?php esc_attr_e('Reset Options', 'complete'); ?></h4>
                <div class="settings_toggle_inner">
                	<p><?php esc_attr_e('Reset Options to default theme settings. All your current theme settings will be lost except the page selection & widgets settings.', 'complete'); ?></p>
                    <form id="complete_reset" method="post" action="" onsubmit="return confirm('<?php esc_attr_e('Do you really want to Reset? All your Theme Settings will be lost.', 'complete'); ?>')">
            ...
            <!--FOOTER LINKS-->
            <div id="footlinks">
            	<a class="optim_dashboard" href="<?php echo admin_url(); ?>" title="<?php esc_attr_e('Close', 'complete'); ?>"><i class="fa fa-times"></i></a>
            	<a class="optim_docs" target="_blank" href="<?php echo esc_url(SKT_THEME_DOC);?>" title="<?php esc_attr_e('Documentation', 'complete'); ?>"><i class="fa fa-book"></i></a>
                <a class="optim_settings" title="<?php esc_attr_e('Settings', 'complete'); ?>"><i class="fa fa-cog"></i></a>
            </div>
...
<?php if(is_customize_preview()) { ?>
        ...
        <div id="customizer_topbar">
            ...
            <label class="current_edit"><?php esc_attr_e('View', 'complete'); ?> 
            	<?php if (is_front_page() ) { ?><a><?php esc_attr_e('Front Page', 'complete'); ?> <i></i></a><?php } ?>
            	<?php if(is_page()){ ?><a><?php echo get_the_title(get_the_ID()); ?> <i></i></a><?php } ?>
                <?php if(is_single()){ ?><a><?php esc_attr_e('Single Post', 'complete'); ?> <i></i></a><?php } ?>
                <?php if(is_category()){ ?><a><?php esc_attr_e('Category Page', 'complete'); ?> <i></i></a><?php } ?>
                <?php if(is_tag()){ ?><a><?php esc_attr_e('Tag Page', 'complete'); ?> <i></i></a><?php } ?>
                <?php if(is_date()){ ?><a><?php esc_attr_e('Archive Page', 'complete'); ?> <i></i></a><?php } ?>
                <?php if(is_author()){ ?><a><?php esc_attr_e('Author Page', 'complete'); ?> <i></i></a><?php } ?>
                <?php if(is_search()){ ?><a><?php esc_attr_e('Search Page', 'complete'); ?> <i></i></a><?php } ?>
                <?php if(is_404()){ ?><a><?php esc_attr_e('404 Page', 'complete'); ?> <i></i></a><?php } ?>
            </label>
                    <ul>
                    	<!--PAGES-->
                    	<li><strong><?php esc_attr_e('Pages:', 'complete'); ?></strong></li>
                        <li><a href="<?php echo home_url('/'); ?>"><?php esc_attr_e('Front Page', 'complete'); ?></a></li>
                        ...
                        <?php } ?>
                        <!--POSTS-->
                        <?php $postid =''; $postq= get_posts("post_type=post&numberposts=1&post_status=publish"); 
							if($postq){
								$postid =$postq[0]->ID; 
							}
						?>
                        <?php
                        $categories = term_exists( 'uncategorized', 'category' );
                         if ( $categories != NULL ) {
							$catid = $categories['term_id'];
						}
						?>
                        <?php $dateid =''; ?>
                        <li><strong><?php esc_attr_e('Posts:', 'complete'); ?></strong></li>
                        <li><a href="<?php echo get_permalink($postid); ?>"><?php esc_attr_e('Single Post', 'complete'); ?></a></li>
                        <li><a href="<?php echo home_url('/?cat=').$catid; ?>"><?php esc_attr_e('Category Page', 'complete'); ?></a></li>
                        <li><a href="<?php echo home_url('/?author=1'); ?>"><?php esc_attr_e('Author Page', 'complete'); ?></a></li>
                        <!--MISC-->
                        <li><strong><?php esc_attr_e('Other:', 'complete'); ?></strong></li>
                        <li><a href="<?php echo home_url('/?s=the'); ?>"><?php esc_attr_e('Search Page', 'complete'); ?></a></li>
                        <li><a href="<?php echo home_url('/nn49721667/'); ?>"><?php esc_attr_e('404 Page', 'complete'); ?></a></li>
                    </ul>
            </div>
        </div>
		<!--CUSTOMIZER LOADER-->
            ...
            <?php if(is_singular()){ ?>
				<?php if(!is_active_sidebar('sidebar')){ $has_sidebar = 'has_no_sidebar';}else{ $has_sidebar = 'has_sidebar';}?>
                    <div class="customizer_sidebar_holder <?php echo $has_sidebar; ?>" data-sidebar-id="sidebar">
                        <a class="add_widget_topage" title="<?php esc_attr_e('Add Widgets Here','complete'); ?>"><i class="fa fa-plus"></i></a>
                    </div>
                <?php } ?>
<?php } ?>
